function [knn_model,Y_tar_pseudo,acc,acc_ite,A] = MyBDA(X_src,Y_src,X_tar,Y_tar,options)
    % Inputs:
	...
	for i = 1 : T
        ...
        [Y_tar_pseudo, KNNScore]     = knn_model.predict(Zt);
        acc = length(find(Y_tar_pseudo==Y_tar))/length(Y_tar); 
        %auc = plotroc(Y_tar,Zt,knn_model);
        %fprintf('Iteration [%2d]:\n',auc);
        %plotroc(Y_tar,Y_tar_pseudo)
