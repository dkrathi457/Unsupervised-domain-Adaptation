function auc = plotroc(y,x,type,name,params)
...
% 	plotroc(y,x,model);
	...
	elseif exist('params') && ~ischar(params)
		...
		auc = roc_curve(deci*model.Label(1),y,type, name);
	else
		if ~exist('params')
			params = [];
		end
		...
		if fold <= 1
			...
			auc = roc_curve(deci,label_y, type, name); % plot ROC curve
		end
	end
end
...
function auc = roc_curve(deci,label_y,type, name)
    ...
    name = [name ' - ' num2str((auc*100), '%0.2f%%')]
    %name = strcat(name, num2str(auc))
	plot(stack_x,stack_y, type, 'DisplayName', name, 'LineWidth' , 2);
	xlabel('False Positive Rate');
	ylabel('True Positive Rate');
	%legend([name '(AUC = ' num2str(auc) ' )'])
end
